...
const pillars = [
  {
    icon: BarChart3, title: 'Evaluation', color: '#00d4ff', bg: 'rgba(0,212,255,0.12)',
  ...
  {
    icon: Brain, title: 'Memory', color: '#a855f7', bg: 'rgba(168,85,247,0.12)',
    items: ['Short-term vs long-term design', 'State boundaries matter', 'Personalization vs confusion', 'Retrieval strategy is critical'],
  },
  {
    icon: TrendingUp, title: 'Optimization', color: '#22c55e', bg: 'rgba(34,197,94,0.12)',
...
export function EvalsMemorySlide() {
  return (
    <SlideLayout>
      ...
      >
        You can't prompt your way into{' '}
        <span className="gradient-text-green">reliability</span>
      </motion.h2>
      ...
      <motion.div variants={item} className="relative z-10 grid grid-cols-3 gap-6 max-w-5xl w-full">
        {pillars.map((p, pi) => (
          <motion.div
            ...
            whileHover={{ scale: 1.03, boxShadow: `0 0 40px ${p.bg}` }}
          >
            ...
            <h3 className="text-2xl font-black mb-5" style={{ color: p.color }}>{p.title}</h3>
      ...
      <motion.div variants={item} className="relative z-10 flex items-center gap-4 mt-12 font-mono">
        {['Build', 'Evaluate', 'Optimize', 'Repeat'].map((s, i) => (
          <span key={s} className="flex items-center gap-4">
            <motion.span
              className={`glass-sm px-5 py-2.5 text-base font-bold ${
                s === 'Repeat' ? 'text-neon-green neon-glow-green' : 'text-text-secondary'
              }`}
