...
export function DSPyGEPASlide() {
  return (
    <SlideLayout>
      <GradientOrb color="rgba(255,255,255,0.04)" size={500} x="25%" y="35%" />
      ...
      <motion.div variants={item} className="relative z-10 grid grid-cols-2 gap-8 max-w-5xl w-full">
        <motion.div className="glass-strong p-8 relative overflow-hidden" whileHover={{ boxShadow: '0 0 30px rgba(255,255,255,0.05)' }}>
          <div className="absolute top-0 left-0 right-0 h-[3px] bg-white/60 rounded-t-3xl" />
          <div className="flex items-center gap-4 mb-5">
            <div className="w-14 h-14 rounded-xl bg-white/8 flex items-center justify-center">
              <Code size={28} className="text-white/90" />
            </div>
            <div>
          ...
          <pre className="code-block mb-6"><code>{`class RAGAgent(dspy.Module):
    def forward(self, question):
        ...
        <motion.div className="glass-strong p-8 relative overflow-hidden" whileHover={{ boxShadow: '0 0 30px rgba(255,255,255,0.05)' }}>
          ...
          <pre className="code-block mb-6"><code>{`for step in range(N):
      ...
      <motion.div variants={item} className="relative z-10 flex items-center gap-3 mt-12 font-mono">
        {['Program', 'Evaluate', 'Reflect', 'Optimize', 'Better System'].map((s, i) => (
          <span key={s} className="flex items-center gap-3">
            <motion.span className={`glass-sm px-4 py-2 text-sm font-bold ${s === 'Better System' ? 'text-white' : 'text-text-secondary'}`} whileHover={{ scale: 1.08 }}>
              {s}
            </motion.span>
            {i < 4 && (
