...
const cards = [
  ...
  },
  {
    ...
    color: '#34A853', bg: 'rgba(52,168,83,0.12)',
...
export function GoogleEcosystemSlide() {
  return (
    <SlideLayout>
      <GradientOrb color="rgba(66,133,244,0.08)" size={700} x="50%" y="45%" />
      ...
      <motion.div variants={item} className="relative z-10 grid grid-cols-3 gap-6 max-w-5xl w-full">
        {cards.map((card, i) => (
          <motion.div
            ...
            whileHover={{ scale: 1.03, boxShadow: `0 0 40px ${card.bg}` }}
          >
      ...
      <motion.div variants={item} className="relative z-10 flex items-center gap-4 mt-12 font-mono">
        {[
          { label: 'Gemini', c: '#4285F4' },
          { label: 'ADK Runtime', c: '#34A853' },
          { label: 'Tools', c: '#FBBC04' },
          { label: 'A2A Network', c: '#EA4335' },
        ].map((s, i) => (
          <span key={s.label} className="flex items-center gap-4">
            <motion.span className="glass-sm px-5 py-2.5 text-sm font-bold"
              style={{ color: s.c }}
              whileHover={{ scale: 1.08, boxShadow: `0 0 20px ${s.c}33` }}>
              {s.label}
            </motion.span>
            {i < 3 && (
